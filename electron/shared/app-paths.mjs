...
export function appDataPaths(env = process.env, platform = process.platform, home = os.homedir()) {
  ...
  let defaultBase;
  if (platform === "win32") {
    defaultBase = targetPath.join(env.LOCALAPPDATA || targetPath.join(home, "AppData", "Local"), "Holocron3D");
  } else if (platform === "darwin") {
    defaultBase = targetPath.join(home, "Library", "Application Support", "Holocron3D");
  } else {
    defaultBase = targetPath.join(env.XDG_DATA_HOME || targetPath.join(home, ".local", "share"), "Holocron3D");
  }
