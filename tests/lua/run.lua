...
for _, name in ipairs(specs) do
  knownSpecs[name] = true
end
local selected, filter
local index = 1
while index <= #arg do
  if arg[index] == "--file" then
    selected = arg[index + 1]
    index = index + 2
  elseif arg[index] == "--filter" then
    filter = arg[index + 1]
    index = index + 2
  else
    error("unknown Lua test option: " .. tostring(arg[index]))
  end
end
if selected then
...
if not harness.run(filter) then
  os.exit(1)
end
