...
local globalNames = {
  "registerAnonymousEventHandler", "tempRegexTrigger", "tempPromptTrigger", "tempTimer",
  "tempTrigger", "killAnonymousEventHandler", "killTrigger", "killTimer", "send", "sendGMCP",
  "deleteLine", "denyCurrentSend", "echo", "cecho", "line", "gmcp", "lotj",
}

local function copyGlobals()
  local values = {}
  for _, name in ipairs(globalNames) do values[name] = _G[name] end
  return values
end

local function restoreGlobals(values)
  for _, name in ipairs(globalNames) do _G[name] = values[name] end
end

function Fixture.new(options)
  options = options or {}
  local self = {
    originalGlobals = copyGlobals(), nextId = 0, events = {}, triggers = {}, timers = {},
    commands = {}, gmcpRequests = {}, snapshots = {}, spaceStates = {}, diagnostics = {},
    intentHandlers = {}, intentAcks = {}, deletedLines = 0, deniedSends = 0,
  }
  ...
  _G.killAnonymousEventHandler = function(value) self.events[value] = nil; return true end
  _G.killTrigger = function(value) self.triggers[value] = nil; return true end
  _G.killTimer = function(value) self.timers[value] = nil; return true end
  ...
  _G.deleteLine = function() self.deletedLines = self.deletedLines + 1 end
  _G.denyCurrentSend = function() self.deniedSends = self.deniedSends + 1 end
  ...
  function self:capture(command, output, sentCommand)
    ...
    for value in (output .. "\n"):gmatch("(.-)\n") do self.scraper.captureLine(value) end
    if self.scraper.active then return self.scraper.finishCapture("fixture") end
  ...
  function self:lastCommand() return self.commands[#self.commands] end
  function self:lastSnapshot() return self.snapshots[#self.snapshots] end
  ...
  function self:close()
    if self.scraper then self.scraper.teardown() end
