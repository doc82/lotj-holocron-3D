...
import { conservativeJumpRange, escapeDestinationInRange, galacticDistance, hyperspaceClearance } from "../renderer/src/domain/hyperspace.ts";
...
test("escape planning uses the ship's conservative confirmed jump range", () => {
  ...
  assert.equal(escapeDestinationInRange({ x: 0, y: 0 }, { x: 30, y: 0 }, destinations).allowed, true);
  assert.equal(escapeDestinationInRange({ x: 0, y: 0 }, { x: 50, y: 0 }, destinations).allowed, false);
});

test("known out-of-range and unconfirmed local escape systems are blocked", () => {
  const outOfRange = escapeDestinationInRange(
    { x: 0, y: 0 }, { x: 10, y: 0 }, destinations, "Wroona System", true,
  );
  assert.equal(outOfRange.allowed, false);
  assert.match(outOfRange.reason, /out of range/i);

  const unknown = escapeDestinationInRange(
    { x: 0, y: 0 }, { x: 10, y: 0 }, destinations, "Private Sector", true,
  );
...
test("hyperdrive clearance requires a fresh spatial fix and 500 units from contacts", () => {
  ...
  const blocked = hyperspaceClearance({ ...base, entities: [
    { id: "frigate", name: "Frigate", kind: "ship", x: 300, y: 0, z: 0 },
  ] }, 1_005);
  assert.equal(blocked.allowed, false);
  assert.equal(blocked.nearestDistance, 300);
  assert.equal(hyperspaceClearance({ ...base, entities: [
    { id: "planet", name: "Planet", kind: "planet", x: 500, y: 0, z: 0 },
  ] }, 1_005).allowed, true);
