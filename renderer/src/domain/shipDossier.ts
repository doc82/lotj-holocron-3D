...
const INFO_CARD_SCHEMA: Array<{ title: string; labels: string[] }> = [
  ...
  { title: "WEAPONS", labels: ["Autoblasters", "Laser Cannons", "Turbolasers", "Ion Cannons",
    "Maximum Missiles", "Maximum Torpedoes", "Maximum Rockets", "Maximum Pulses", "Maximum Chaff",
    "Missile Tubes", "Tractor Beams", "Escape Pods"] },
  { title: "ACCESS CODES", labels: ["Hatchway", "Hangar Bays", "Docking", "Self-destruct"] },
  { title: "SYSTEMS", labels: ["Maximum Hull", "Maximum Shields", "Maximum Energy (fuel)",
    "Maximum Speed", "Hyperspeed", "Maneuver", "Sensor Array", "Shield Boosters",
    "Communications", "Cloaking Device"] },
];

export function validatedInfoSections(sections: ShipCardSection[] = []): ShipCardSection[] {
  return INFO_CARD_SCHEMA.flatMap((definition) => {
    const source = sections.find((section) => section.title?.trim().toUpperCase() === definition.title);
    if (!source) return [];
    const rows = new Map(source.rows.flatMap((row) => {
      const label = String(row.label || "").trim();
      const value = String(row.value || "").replace(/\s+/g, " ").trim();
      return definition.labels.includes(label) && value !== "" && value.length <= 100
          && !/[{}\r\n]/.test(value)
