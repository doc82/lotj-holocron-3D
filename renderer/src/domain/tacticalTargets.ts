...
function ownerLabel(target: CombatTargetTrack): string {
  return target.ownerLabel
    || (target.scope === "local" ? "YOUR SHIP'S TARGET"
      : target.scope === "all" ? "FLEET TARGET"
        : target.scope === "wings" ? "WING TARGET"
          : target.scope === "squadron" ? "SQUADRON TARGET"
...
export function buildTacticalTargetShortcuts({
  ...
  if (fallbackLocalTarget !== "" && fallbackLocalTarget.toLowerCase() !== "none"
