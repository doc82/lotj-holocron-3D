...
function vector(value: unknown): string | null {
  ...
  return [coordinates.x, coordinates.y, coordinates.z]
    .map((coordinate) => (Number.isFinite(coordinate) ? coordinate : "?"))
    .join("  ");
}
...
function fallbackSections(ship: TelemetryEntity, mode: ShipDossierMode): ShipCardSection[] {
  if (mode === "status") {
    ...
    return [
      { title: "FLIGHT", rows: flight },
      { title: "SYSTEMS", rows: systems },
    ];
  }
  ...
  const weaponRows = Object.entries(
    ship.weapons && typeof ship.weapons === "object"
      ? (ship.weapons as Record<string, unknown>)
      : {},
  ).map(([label, value]) => ({
...
export function ShipDossierPanel({
  ship,
  mode,
  loading,
  message,
  ...
  const cardSections =
    mode === "info"
      ...
      : sanitizedStatusSections(card?.sections);
  ...
  return (
    <aside className={styles.dossier} aria-label={`${mode} card for ${ship.name || "ship"}`}>
      ...
      <div className={styles.body}>
        {!card && (
          <div className={styles.pending} data-loading={loading}>
