...
function TargetMeter({ label, value, tone }: {
  label: string;
  value: number | null;
  tone: "hull" | "shield" | "energy";
}) {
  const tooltip = `${label} // ${value === null ? "UNKNOWN" : `${Math.round(value)}%`}`;
  return <span className={rosterStyles.meter} title={tooltip} aria-label={tooltip}>
    <span>{label.slice(0, 1)}</span>
    <i><b className={rosterStyles[tone]} style={{ width: `${value ?? 0}%` }} /></i>
  </span>;
}

function TargetCard({ target, onFocus, onOpenDossier }: {
  ...
  const disabled = String(ship?.condition || "").trim().toLowerCase() === "disabled";
  const activate = () => { if (available) onFocus(target); };
  return <article className={`${rosterStyles.member} ${styles.targetCard}`}
    data-disabled={disabled} data-available={available}
    aria-label={`${target.ownerLabels.join(", ")}: ${target.targetName}`}
    role="button" tabIndex={available ? 0 : -1}
    onClick={activate}
    onKeyDown={(event) => {
    ...
    <span className={rosterStyles.memberTop}>
      ...
      {ship && <span className={rosterStyles.cardActions}>
        <button type="button" aria-label={`Show status card for ${target.targetName}`}
          onClick={(event) => { event.stopPropagation(); onOpenDossier(ship, "status"); }}>S</button>
        <button type="button" aria-label={`Show info card for ${target.targetName}`}
          onClick={(event) => { event.stopPropagation(); onOpenDossier(ship, "info"); }}>I</button>
      </span>}
    </span>
    ...
    <small>{ship
      ...
      : "CONTACT NOT PRESENT IN CURRENT SENSOR PICTURE"}</small>
    {disabled && <small className={rosterStyles.disabledState}>DISABLED // SYSTEMS FAILURE</small>}
    {ship && <span className={rosterStyles.meters}>
...
export function TargetShortcutRail({ targets, drawerOpen, onToggle, onFocus, onOpenDossier }: {
  ...
  return <>
    <nav className={styles.rail} aria-label="Active combat targets">
      <p>TARGETS</p>
      <button type="button" aria-pressed={drawerOpen} aria-expanded={targets.length > 1 && drawerOpen}
        aria-label={directTarget ? `Select ${directTarget.targetName}` : `Choose from ${targets.length} active targets`}
    ...
    {targets.length > 1 && drawerOpen && <aside className={styles.drawer} aria-label="Active target shortcuts">
      <header>
        <div><p>TARGET MEMORY</p><h2>{targets.length} ACTIVE TARGETS</h2></div>
        <button type="button" className={styles.close} aria-label="Close active target shortcuts"
          onClick={onToggle}>×</button>
      </header>
      <div className={styles.list}>
        {targets.map((target) => <TargetCard key={target.id} target={target}
          onFocus={onFocus} onOpenDossier={onOpenDossier} />)}
      </div>
    </aside>}
  </>;
}
