...
export const WEAPONS: WeaponDefinition[] = [
  {
    ...
    label: "AUTOBLASTER",
    field: "autoblasters",
    description: "Rapid close-range energy fire",
  },
  {
    type: "laser",
    label: "LASER",
    field: "laserCannons",
    description: "Focused general-purpose energy fire",
  },
  {
    type: "turbolaser",
    label: "TURBOLASER",
    field: "turbolasers",
    description: "Heavy capital-ship energy fire",
  },
  {
    type: "ion",
    label: "ION",
    field: "ionCannons",
    description: "Ion fire against shields and systems",
  },
  {
    type: "missile",
    label: "MISSILE",
    ...
    launcher: true,
    ammoField: "missiles",
    description: "Guided explosive projectile",
  },
  {
    type: "torpedo",
    label: "TORPEDO",
    ...
    launcher: true,
    ammoField: "torpedoes",
    description: "Heavy guided projectile",
  },
  {
    type: "rocket",
    label: "ROCKET",
    field: "maximumRockets",
    launcher: true,
    ammoField: "rockets",
    description: "Fast unguided projectile",
  },
  {
    type: "burst",
    label: "BURST",
    field: "maximumPulses",
    launcher: true,
    description: "Pulse launcher burst",
  },
];
  ...
  const paths = {
    all: (
      <>
        ...
        <path d="m21 3 6 4-6 4M21 12l6 4-6 4M21 21l6 4-6 4" />
      </>
    ),
    best: (
      <>
    ...
    autoblaster: (
      <>
        <path d="M5 9h13M5 16h18M5 23h13" />
        <path d="m18 6 7 3-7 3M23 13l6 3-6 3M18 20l7 3-7 3" />
      </>
    ),
    laser: (
      <>
        <path d="M4 22 22 8" />
        <path d="m18 6 8-1-2 8M7 25l-2 2" />
      </>
    ),
    turbolaser: (
      <>
        <path d="M4 11h20M4 21h20" />
        <path d="m20 6 8 5-8 5M20 16l8 5-8 5" />
      </>
    ),
    ion: (
      <>
        <circle cx="16" cy="16" r="10" />
        <path d="m18 5-7 11h6l-3 11 8-14h-6z" />
      </>
    ),
    missile: (
      <>
        <path d="M7 24 20 7l5 1 1 5-17 13z" />
        <path d="m9 20-5 1 3-5M13 24l-1 5-4-3" />
      </>
    ),
    torpedo: (
      <>
        <path d="M5 16 14 7h8l6 9-6 9h-8z" />
        <path d="M5 11v10M14 7l3 9-3 9" />
      </>
    ),
    rocket: (
      <>
        <path d="M9 22 19 6l6 1 1 6-16 10z" />
        <path d="m11 19-6 7M15 22l-3 6" />
      </>
    ),
    burst: (
      <>
        <circle cx="16" cy="16" r="4" />
        <path d="M16 2v8M16 22v8M2 16h8M22 16h8M6 6l6 6M20 20l6 6M26 6l-6 6M12 20l-6 6" />
      </>
    ),
      ...
      if (event.type === "launch") {
        ...
        setStatus(
          `${event.weapon.toUpperCase()} // ${
            event.outcome === "hit" ? `${count} TARGET HIT${count === 1 ? "" : "S"}` : "SHOT MISSED"
          }`,
        );
        ...
        setStatus(
          `${event.weapon.toUpperCase()} // ${launcher ? "LAUNCHER RELOADED" : "FULLY CHARGED"}`,
        );
        ...
        setStatus(
          `${event.weapon.toUpperCase()} // ${(event.reason || "FIRE CONTROL LOCK FAILED").toUpperCase()}`,
        );
  ...
  return (
    <section
      className={`${styles.panel} ${
        rumbleToken > 0 ? (rumbleToken % 2 === 0 ? styles.rumbleEven : styles.rumbleOdd) : ""
      }`}
    ...
    >
      ...
      <div className={styles.weaponGrid}>
        <button
          type="button"
          disabled={disabled}
          aria-label="Fire all installed weapons"
          data-tooltip="FIRE ALL // Cycle every installed weapon"
        ...
        <button
          type="button"
          disabled={disabled}
          aria-label="Fire best available weapon"
          data-tooltip="BEST WEAPON // Let LotJ select the best available weapon"
        ...
        {WEAPONS.map((weapon) => {
          ...
          const ammunition =
            weapon.ammoField && typeof observer[weapon.ammoField] === "object"
          ...
          return (
            ...
            >
              ...
              <span>{recharging ? (weapon.launcher ? "RELOADING" : "CHARGING") : count}</span>
            </button>
          );
        })}
      </div>
