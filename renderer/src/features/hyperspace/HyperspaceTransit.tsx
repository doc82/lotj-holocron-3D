...
export function HyperspaceTransit({ reentry, escapePending, onEscape }: Props) {
  ...
  return <section className={`${styles.transit} ${reentry ? styles.reentry : ""}`}
    style={reentry ? {
      animation: "none",
    ...
    {!reentry && <div className={styles.emergencyHousing} data-tooltip="ESCAPE HYPERSPACE">
      <span>EMERGENCY HYPERDRIVE CUTOFF</span>
      <button type="button" aria-label="Escape hyperspace" title="Escape hyperspace"
        disabled={escapePending} onClick={onEscape}>
        <i aria-hidden="true" />
      </button>
