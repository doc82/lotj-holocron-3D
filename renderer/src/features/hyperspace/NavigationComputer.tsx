...
export function NavigationComputer({
  route,
  state,
  escape,
  clearance,
  ...
  return (
    <>
      <aside
        className={`${styles.computer} ${styles[phase]}`}
        aria-label="Hyperspace navigation computer"
      >
        <header>
        ...
        <div className={styles.recipient}>
          ROUTE FOR // {(route.recipientLabel || "YOUR SHIP").toUpperCase()}
        </div>
        <h3>
          {route.mode === "local"
            ? "LOCAL JUMP"
            : route.systemName || `GX ${route.galaxy?.x} / ${route.galaxy?.y}`}
        </h3>
        <p>
        ...
        {escape && (
          <div className={styles.escape}>
            <span>ESCAPE PLAN ARMED</span>
            <b>
              GX {escape.route.galaxy?.x} / {escape.route.galaxy?.y}
            </b>
          </div>
        )}
        {phase === "ready" && (
          <div
            className={`${styles.clearance} ${!clearance.known ? styles.clearancePending : clearance.allowed ? styles.clearanceSafe : styles.clearanceBlocked}`}
          >
            {!clearance.known
