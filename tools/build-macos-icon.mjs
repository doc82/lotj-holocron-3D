...
if (fs.existsSync(destination) && fs.statSync(destination).mtimeMs >= fs.statSync(source).mtimeMs) process.exit(0);
...
try {
  for (const size of [16, 32, 128, 256, 512]) {
    for (const scale of [1, 2]) {
      ...
      const resized = spawnSync("sips", ["-z", String(pixels), String(pixels), source, "--out", output], { stdio: "inherit" });
      if (resized.status !== 0) process.exit(resized.status ?? 1);
    }
  }
  const converted = spawnSync("iconutil", ["-c", "icns", iconset, "-o", destination], { stdio: "inherit" });
